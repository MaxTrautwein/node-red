...
function Node(n) {
    ...
    this.full_wires = n.full_wires[0];
    
    
    
  /*  console.log("-----------------------------------------------------");
    DebugObject(this.full_wires);
    console.log("-----------------------------------------------------");*/
    if (n.name) {
        this.name = n.name;
    }
...
Node.prototype.send = function(msg) {
    ...
    //Pront Contents of msg object
    //This contains in case of the Inject Node the foolowing information:
    /*
     Sending: [object Object]
    msg[_msgid] --> a1584dd887ea990a
    msg[payload] --> Top
    msg[topic] -->
    -----------------------------------------------------------------
    */
   //_msgid --> Identifies that message
   //payload --> The Message sent String or number or wahtever was specified
   //topic --> if specified
    ...
    if (msg === null || typeof msg === "undefined") {
        ...
        if (this._wire) {
            ...
            console.log("this._flow.send: " + this.id);
            //Inject TragetPort
            
            /*var _links = this.full_wires;
            for(var key in _links) {
                var value = _links[key];
                if (typeof value === 'object' && value !== null){
                    for(var key2 in value) {
                        var value2 = value[key2];
                        console.log("_links[" + key + "]  --> " + " value[" + key2 + "]  --> " + value2);
                    }
                }else{
                    console.log("_links[" + key+ "] --> " + value);
                } 
            ...
            */
            //
            msg["__port"] = this.full_wires[0]["target_port"];
            /*
            console.log("-------------------------This-----------------------------");
            for(var key in this) {
                var value = this[key];
                ...
                for (var j = 0; j < wires.length; j++) {
                    ...
                    for (k = 0; k < msgs.length; k++) {
                        ...
                        if (m !== null && m !== undefined) {
                            ...
                            console.log("this._flow.send2: " + m + "  from: " + this.id);
                            //console.log("target port: " + this.full_wires[j]["target_port"]);
                            msg["__port"] = this.full_wires[j]["target_port"];
                            sendEvents.push({
                                msg: m,
                                source: {
                                    id: this.id,
