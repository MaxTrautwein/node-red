...
RED.nodes = (function() {
    ...
    function addLink(l) {
        ...
        for(var key in l) {
            ...
            if (typeof value === 'object' && value !== null){
                for(var key2 in value) {
                    ...
                    console.log("l[" + key + "]  --> " + " value[" + key2 + "]  --> " + value2);
                }
            }else{
        ...
        */
        ...
        if (l.source) {
            ...
            console.log("New out link for: " + l.source.id);
        ...
        if (l.target) {
            ...
            console.log("New in link for: " + l.target.id);
            nodeLinks[l.target.id].in.push(l);
        }
        if (l.source.z === l.target.z && linkTabMap[l.source.z]) {
            console.log("New link pair in flow: " + l.source.z);
    ...
    function convertNode(n, opts) {
        ...
        if (n._def.category != "config") {
            ...
            console.log("n._def.category != 'config' for: " + node.id);
            node.wires = [];
            //Somhow this is not returned to the node???
            node.full_wires = [];
            for(var i=0;i<n.outputs;i++) {
                node.wires.push([]);
                //console.log("Added stuff to full_wires 1");
                node.full_wires.push([]);
            }
            ...
            for (var j=0;j<wires.length;j++) {
                ...
                if (w.target.type != "subflow") {
                    if (w.sourcePort < node.wires.length) {
                        ...
                        console.log("Added stuff to full_wires 2 - ");
                        //w includes all the information that i need / want
                        //console.log("node.full_wires[" + w.sourcePort + "].push(" + w + ");");
                        /*
                        console.log("---------------------------Find the circularReference ------------------------------");
                        for(var key in w) {
                            var value = w[key];
                            if (typeof value === 'object' && value !== null){
                                for(var key2 in value) {
                                    var value2 = value[key2];
                                    console.log("w[" + key + "]  --> " + " value[" + key2 + "]  --> " + value2);
                                }
                            }else{
                                console.log("w[" + key+ "] --> " + value);
                            } 
                        ...
                        console.log("----------------------------------------------------------------------------------------------");
                        */
                        var newdatat = w.target_port;


                        node.full_wires[w.sourcePort].push({"target_port":newdatat});
                    }
                }
            }
        ...
        for(var key in node) {
            ...
            if (typeof value === 'object' && value !== null){
                for(var key2 in value) {
                    var value2 = value[key2];
                    console.log("node[" + key + "]  --> " + " value[" + key2 + "]  --> " + value2);
                }
            }else{
        ...
        //This return seemes to not update full_wires for the node itself
        //However this iformation can be found in the json of this flow / node
        return node;
    }
    ...
    //TAR TODO Smehow this dos not re set full_wires
    /**
     * Options:
    ...
    function importNodes(newNodesObj,options) { // createNewIds,createMissingWorkspace) {
        ...
        //newNodes Contains the information
        ...
        for (i=0;i<newNodes.length;i++) {
            n = newNodes[i];
            //
            //This is each nodes / flows data
            //The full_wires Data is seen here
            console.log("newNodes iteration: " + i + "  -------------------------------------");
            DebugObject(n,"");
            //
        ...
        for (i=0;i<newNodes.length;i++) {
            ...
            if (def && def.category == "config") {
                ...
                if (!existingConfigNode || existingConfigNode._def.exclusive) { //} || !compareNodes(existingConfigNode,n,true) || existingConfigNode.z !== n.z) {
                    ...
                    console.log("Added new configNode");
        ...
        for (i=0;i<newNodes.length;i++) {
            ...
            if (n.type !== "workspace" && n.type !== "tab" && n.type !== "subflow") {
                ...
                if (!def || def.category != "config") {
                    ...
                    if (n.type !== "group") {
                        node.wires = n.wires||[];
                        node.full_wires = n.full_wires[0];
                        console.log("node.full_wires: " + node.full_wires);
                    ...
                    for (var w2=0;w2<wires.length;w2++) {
                        if (node_map.hasOwnProperty(wires[w2])) {
                            if (n.z === node_map[wires[w2]].z) {
                                var targetport = n.full_wires[w2].target_port || 0;
                                var link = {source:n,sourcePort:w1,target:node_map[wires[w2]],target_port:targetport};
                                addLink(link);
                                new_links.push(link);
                            } else {
        ...
        //TODO include Dest Port
        console.log("Construct link from wire Subflow cnt:  " + new_subflows.length);
        ...
        //DebugObject(new_links,"");
        //new links contains the remap
        return {
            nodes:new_nodes,
            links:new_links,
            groups:new_groups,
