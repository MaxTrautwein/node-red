...
RED.view = (function() {
    ...
    function addNode(type,x,y) {
        ...
        nn.w = node_width;
        var maxPorts = Math.max(nn.outputs||0,nn.inputs||0)
        nn.h = Math.max(node_height,(maxPorts) * 15);
        nn.resize = true;
    ...
    function showQuickAddDialog(options) {
        ...
        var rebuildQuickAddLink = function() {
            ...
            quickAddLink.el.attr("d",generateLinkPath(quickAddLink.node.x+sc*quickAddLink.node.w/2,quickAddLink.node.y+portY,
                point[0]-sc*node_width/2, point[1]
                ,sc));
        }
        if (quickAddLink) {
            rebuildQuickAddLink();
    ...
    function canvasMouseMove() {
        ...
        if (mouse_mode == RED.state.JOINING || mouse_mode === RED.state.QUICK_JOINING) {
            ...
            for (i=0;i<drag_lines.length;i++) {
                ...
                drag_line.el.attr("d",generateLinkPath(drag_line.node.x+sc*drag_line.node.w/2,drag_line.node.y+portY,
            ...
            for (i=0;i<drag_lines.length;i++) {
                if (portType != drag_lines[i].portType && mouseup_node !== drag_lines[i].node) {
                    ...
                    if (drag_line.portType === PORT_TYPE_OUTPUT) {
                        src = drag_line.node;
                        src_port = drag_line.port;
                        dst_port = portIndex; //This is correct
                        //console.log("drag_line.portType === PORT_TYPE_OUTPUT - src_port: " + src_port);
                        //console.log("portIdex: " + portIndex);
                        dst = mouseup_node;
                    } else if (drag_line.portType === PORT_TYPE_INPUT) {
                        src = mouseup_node;
                        dst = drag_line.node;
                        src_port = portIndex;
                        dst_port = drag_line.port; //This is correct
                        //console.log("drag_line.port: " + drag_line.port);
                        //console.log("drag_line.portType === PORT_TYPE_INPUT - src_port: " + src_port);
                    }
                    var link = {source: src, sourcePort:src_port, target: dst, target_port:dst_port};
                    if (drag_line.virtualLink) {
                        ...
                        if (!(
                            ...
                            if (!existingLink) {
                                console.log("Add link: " + link.source.id + " >> " + link.sourcePort + "  |  " + link.target.id + " >> " + link.target_port);
            ...
            node.each(function(d,i) {
                if (d.dirty) {
                    ...
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__outputs__.length !== d.outputs) {
                        ...
                        var maxPorts = Math.max((d.outputs || 0,(d.inputs || 0)));
                        if (hideLabel) {
                            d.h = Math.max(node_height,(maxPorts) * 15);
                        } else {
                            d.h = Math.max(6+24*labelParts.lines.length,(maxPorts) * 15, 30);
                        }
                           ...
                           // console.log(d);
                            for(var portIndex = 0; portIndex < numInputs; portIndex++ ) {
                               ...
                               // if (portIndex === this.__outputs__.length) {
                                    ...
                                    /*
                                    portPort.addEventListener("mousedown",function(d){portMouseDown(d,PORT_TYPE_INPUT,0);});
                                    portPort.addEventListener("touchstart",function(d){portMouseDown(d,PORT_TYPE_INPUT,0);d3.event.preventDefault();});
                                    portPort.addEventListener("mouseup",function(d){portMouseUp(d,PORT_TYPE_INPUT,0);} );
                                    portPort.addEventListener("touchend",function(d){portMouseUp(d,PORT_TYPE_INPUT,0);d3.event.preventDefault();} );
                                    portPort.addEventListener("mouseover",function(d){portMouseOver(d3.select(this),d,PORT_TYPE_INPUT,0);});
                                    portPort.addEventListener("mouseout",function(d) {portMouseOut(d3.select(this),d,PORT_TYPE_INPUT,0);});
                                    */
                                    RED.hooks.trigger("viewAddPort",{node:d,el: this, port: portGroup, portType: "input", portIndex: portIndex})
                              /*  }else{
                                    console.log("Else");
                                    console.log("this.__inputs__[portIndex] " + this.__inputs__);
                                ...
                                /*console.log("translate("+x+","+((y+13*portIndex)-5)+")");
                                console.log(d.h);
                                console.log(portIndex);
                                console.log(((y+13*portIndex)-5));*/
                                //console.log(d);
...
//TAR CHECK DO STUFF WITH PORTS...
            ...
            link.each(function(d) {
                ...
                if (d.added || d===selected_link || d.selected || dirtyNodes[d.source.id] || dirtyNodes[d.target.id]) {
                    ...
                    //Adjust the input location
                    var targetInputs = d.target.inputs || 1;
                    var targetPort = d.target_port || 0
                    var y = -((numOutputs-1)/2)*13 +13*sourcePort;
                    var in_y = -((targetInputs-1)/2)*13 +13*targetPort;
                    
                    d.x1 = d.source.x+d.source.w/2;
                    d.y1 = d.source.y+y;
                    d.x2 = d.target.x-d.target.w/2;
                    d.y2 = d.target.y + in_y;

                    // return "M "+d.x1+" "+d.y1+
                    //     " C "+(d.x1+scale*node_width)+" "+(d.y1+scaleY*node_height)+" "+
                    //     (d.x2-scale*node_width)+" "+(d.y2-scaleY*node_height)+" "+
                    //     d.x2+" "+d.y2;
                    var path = generateLinkPath(d.x1,d.y1,
                        d.x2,d.y2
                        ,1);
                    if (/NaN/.test(path)) {
                        path = ""
